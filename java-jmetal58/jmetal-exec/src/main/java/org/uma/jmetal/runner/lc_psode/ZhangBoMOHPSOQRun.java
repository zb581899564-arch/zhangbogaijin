...
public class ZhangBoMOHPSOQRun extends AbstractAlgorithmRunner {

    private static String algorithmName = new String("ZhangBo-MOHPSO-Q");
    ...
    public static void mainexe(PermutationProblem<PermutationSolution<Integer>> problem,
        ...
        mainexeWithGlobalSearch(problem, numberOfJobs, numberOfStages, numberOffactories,
                problemId, swarmSize, rangeOfR1R2, crossoverRate, mutationRate,
                pmxCrossover, V1mutationProbability, DERate, DEcrossoverRates,
                DEmutationRate, maxIterations, outputSolutionFolderName, indexFileName,
                runIndex, Nonparametric, crossoverRates4worker, crossoverRates4machine,
                mutationRate4worker, mutationRate4machine, localsearch,
                ZhangBoGlobalSearchConfiguration.disabled());
    }

    public static void mainexeWithGlobalSearch(PermutationProblem<PermutationSolution<Integer>> problem,
                               int numberOfJobs,int numberOfStages, int numberOffactories, int problemId,
                               int swarmSize,
                               double rangeOfR1R2,
                               double crossoverRate,double mutationRate,
                               ...
                               ZhangBoGlobalSearchConfiguration globalSearchConfiguration) throws Exception {
        if (globalSearchConfiguration == null) {
            throw new IllegalArgumentException("globalSearchConfiguration cannot be null");
        }
        String activeAlgorithmName = globalSearchConfiguration.isQgEnabled()
                ? algorithmName + "-" + globalSearchConfiguration.getGlobalLeaderMode() + "-"
                    + globalSearchConfiguration.getParticleUpdateMode()
                : algorithmName;
        if (globalSearchConfiguration.isEvaluatedPddrEnabled()) {
            activeAlgorithmName += "-" + globalSearchConfiguration.getEnvironmentalSelectionMode();
        }
        if (globalSearchConfiguration.isLineageArchiveEnabled()) {
            activeAlgorithmName += "-LINEAGE_ARCHIVE";
        }
        //public static void mainexe(PermutationProblem<PermutationSolution<Integer>> problem,
        ...
        if (Nonparametric == false) {
            ...
            for (int i = 0; i < runIndex; i++) {
                ...
                algorithm = new ZhangBoMOHPSOQBuilder(problem, swarmSize, numberOffactories,DERate, DEcrossoverRates, DEmutationRate,pmxCrossover)
                        ...
                        .setGlobalSearchConfiguration(globalSearchConfiguration)
                        .build();     //大概就是到构造函数处
...
//                utilDominationRelationship.outputSolution2FileNonparametric(crossoverRate, mutationRate, rangeOfR1R2, pmxCrossover, V1mutationProbability, swarmSize, DERate, DEcrossoverRates, DEmutationRate, population, algorithmName, numberOfJobs, numberOfStages, numberOffactories, problemId, outputSolutionFolderName);
                utilDominationRelationship.outputSolution2FileNonparametricnew(crossoverRate, mutationRate, rangeOfR1R2, pmxCrossover, V1mutationProbability, swarmSize, DERate, DEcrossoverRates, DEmutationRate, population, activeAlgorithmName, numberOfJobs, numberOfStages, numberOffactories, problemId, outputSolutionFolderName,
                ...
                sbFileName.append(activeAlgorithmName).append("_");
            ...
            utilDominationRelationship.outputCPUTime(avgtime, activeAlgorithmName, numberOfJobs, numberOfStages, numberOffactories, problemId, outputSolutionFolderName);
        }
...
//                System.out.println(population.size());
                utilDominationRelationship.outputSolution2FileNonparametric(crossoverRate, mutationRate, rangeOfR1R2, pmxCrossover, V1mutationProbability, swarmSize, DERate, DEcrossoverRates, DEmutationRate, population, activeAlgorithmName, numberOfJobs, numberOfStages, numberOffactories, problemId, outputSolutionFolderName);
                ...
                sbFileName.append(activeAlgorithmName);
/*                sbFileName.append(rangeOfR1R2).append("_");
                sbFileName.append(pmxCrossover).append("_");   //v1
                sbFileName.append(V1mutationProbability).append("_");    //v1
                sbFileName.append(swarmSize).append("_");
