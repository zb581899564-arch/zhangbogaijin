...
import org.uma.jmetal.algorithm.multiobjective.mypso.util.*;
...
public class MOPSODSDE_Kmeans extends AbstractParticleSwarmOptimization<PermutationSolution<Integer>, List<PermutationSolution<Integer>>> {
    ...
    private double gamma = 0.75, tl = 0.8;
    /**
     * Constructor
     */
    public MOPSODSDE_Kmeans(double crossoverRate, double mutationRate, double rand_k,
                         Problem<PermutationSolution<Integer>> problem, SolutionListEvaluator<PermutationSolution<Integer>> evaluator,
                         int swarmSize, int maxIterations, int upSize, int centralSize, int downSize, double DERate,double DEcrossoverRates,double DEmutationRate,double Krate) {
        ...
        Pd = new ArrayList<List<PermutationSolution<Integer>>>(swarmSize);   // 两层容器
        Pgd = new ArrayList<PermutationSolution<Integer>>();    //一层容器
    ...
    public MOPSODSDE_Kmeans(SolutionListEvaluator<PermutationSolution<Integer>> evaluator) {
        this.evaluator = evaluator;
    }
    ...
    protected List<PermutationSolution<Integer>> createInitialSwarm() {      //创建初始种群
        ...
        int swarmSize2 = (int) (swarmSize*Krate);
        int swarmSize1 = swarmSize - (int) (swarmSize*Krate);
...
/*        for (int t = 0; t < swarmSize2; t++) {
    ...
    protected List<PermutationSolution<Integer>> evaluateSwarm(List<PermutationSolution<Integer>> swarm) {    //评估
        swarm = evaluator.evaluate(swarm, (Problem<PermutationSolution<Integer>>) problem);    //  具体实现在DFSP3
        //currentIteration=currentIteration+swarmSize;
        return swarm;
    }
    ...
    protected void initializeLeader(List<PermutationSolution<Integer>> swarm) {
        for (int i = 0; i < swarmSize; i++) {
            Pgd.add(Pd.get(i).get(0));  //Pd 是arraylist  Pgd是list      // 初始化全局最优解  //历史最优的第一个作为全局
    ...
    protected void updateVelocity(List<PermutationSolution<Integer>> swarm) {           //分群更新速度
        ...
        for (int i = 0; i < swarmSize; i++) {
            ...
            ArrayList<PermutationSolution<Integer>> A = new ArrayList<>(Pd.get(i).size());    // A用于存粒子的每一个历史最优
            ArrayList<int[]> B = new ArrayList<int[]>(Pdflag.get(i).size());

            for (int j = 0; j < Pd.get(i).size(); j++) {

                A.add((PermutationSolution<Integer>) Pd.get(i).get(j).copy());    //存每一个粒子的所有历史最优解    单层
...
// 分完群进行选择
    ...
    private void select() {
        ...
        //判断选择全局最优解
        int a1 = randomGenerator.nextInt(0, Pgd.size() - 1);
        int a2 = randomGenerator.nextInt(0, Pgd.size() - 1);
        int a3 = randomGenerator.nextInt(0, Pgd.size() - 1);
        int temp = a1;
        if (Pgd.get(a1).getObjective(0) >= Pgd.get(a2).getObjective(0) &&
                Pgd.get(a3).getObjective(0) >= Pgd.get(a2).getObjective(0)) {
            temp = a2;
        }

        if (Pgd.get(a1).getObjective(0) >= Pgd.get(a3).getObjective(0) &&
                Pgd.get(a2).getObjective(0) >= Pgd.get(a3).getObjective(0)) {
            temp = a3;
        }
        bb4.add(Pgd.get(temp));
        // Pgdflag.get(temp)[0]++;

        a1 = randomGenerator.nextInt(0, Pgd.size() - 1);
        a2 = randomGenerator.nextInt(0, Pgd.size() - 1);
        a3 = randomGenerator.nextInt(0, Pgd.size() - 1);
        temp = a1;
        List<Double> cc = new ArrayList<>(Pgd.size());
        for (int i = 0; i < Pgd.size(); i++) {
            double count1 = 0;
            double count2 = 0;
            for (int j = 0; j < Pgd.size(); j++) {
                if (i != j) {
...
//
        ...
        a3 = randomGenerator.nextInt(0, Pgd.size() - 1);
        temp = a1;
        if (Pgd.get(a1).getObjective(1) >= Pgd.get(a2).getObjective(1) &&
                Pgd.get(a3).getObjective(1) >= Pgd.get(a2).getObjective(1)) {
            temp = a2;
        }

        if (Pgd.get(a1).getObjective(1) >= Pgd.get(a3).getObjective(1) &&
                Pgd.get(a2).getObjective(1) >= Pgd.get(a3).getObjective(1)) {
            temp = a3;
        }
        bb4.add(Pgd.get(temp));
        // Pgdflag.get(temp)[0]++;
...
//历史最优
    ...
    protected void updatePosition(List<PermutationSolution<Integer>> swarm) {
        ...
        for (int i = 0; i < upSize; i++) {
            ...
            ArrayList<SO> vtemp1 = getDifferenceOfJobSequenceVectorByExchangeSequence(bb1.get(i), particle);
            len = (int) (vtemp1.size() * r1);
            ...
            ArrayList<SO> vtemp2 = getDifferenceOfJobSequenceVectorByExchangeSequence(bb4.get(0), particle);
            len = (int) (vtemp2.size() * r2);
            ...
            m = random.nextDouble();
            if (c < Cross_c) {
                /*ArrayList<SO> vtempa1 = getDifferenceOfFactoryVectorByExchangeSequence(bb4.get(0), particle);
            ...
            //sequence3 s4=new sequence3();
            //fac1(particle,listV2);
            if (m < Mutation_m) {
        ...
        for (int i = 0; i < centralSize; i++) {
            ...
            ArrayList<SO> vtemp1 = getDifferenceOfJobSequenceVectorByExchangeSequence(bb2.get(i), particle);
            len = (int) (vtemp1.size() * r1);
            ...
            ArrayList<SO> vtemp2 = getDifferenceOfJobSequenceVectorByExchangeSequence(bb4.get(1), particle);
            len = (int) (vtemp2.size() * r2);
        ...
        for (int i = 0; i < downSize; i++) {
            ...
            ArrayList<SO> vtemp1 = getDifferenceOfJobSequenceVectorByExchangeSequence(bb3.get(i), particle);
            len = (int) (vtemp1.size() * r1);
            for (int j = 0; j < len; j++) {
                listV.add(vtemp1.get(j));
            }
            ...
            ArrayList<SO> vtemp2 = getDifferenceOfJobSequenceVectorByExchangeSequence(bb4.get(2), particle);
            len = (int) (vtemp2.size() * r2);
    ...
    protected void perturbation(List<PermutationSolution<Integer>> swarm) {
       ...
       /* //在分群后加DE操作
        ...
        for (int i = 0; i < swarm.size(); i++) {
            ...
            swarmtemp.add(DEJobFactory(swarm, DErate, select));       //工件工厂同时操作
            DEswarmtempPdflag[i] = select[1];     //不好的那一个
...
/*        for (int i = 0; i < swarm.size(); i++) {
        ...
        for (int i = 0; i < upSize; i++) {
            ...
            List<PermutationSolution<Integer>> temp = new ArrayList<PermutationSolution<Integer>>(30);
            for (int k = 0; k < 30; k++) {

                double p = random.nextDouble();
                if (k == 0) {
                    actionIndex = random.nextInt(action.size());
                    getswarm1=learn(actionIndex, R, Q, sub1.get(i), next,group);
                    int Qiannext = next;
                    if ((getswarm1.getObjective(0) < sub1.get(i).getObjective(0) && getswarm1.getObjective(1) < sub1.get(i).getObjective(1)) || getswarm1.getObjective(0) < sub1.get(i).getObjective(0) ||getswarm1.getObjective(1) < sub1.get(i).getObjective(1)) next = 0;
                    else next = 1;
                    old0=sub1.get(i).getObjective(0);old1=sub1.get(i).getObjective(1);
                    new0=getswarm1.getObjective(0);new1=getswarm1.getObjective(1);
                    //Q[Qiannext][actionIndex] = calculateNewQ(R, Q,actionIndex,next,Qiannext);
                    Q[Qiannext][actionIndex] = calculateNewQ1(R, Q,actionIndex,next,Qiannext,old0,old1,new0,new1);
                } else {
                    if (p < 1 - tl) {
                        actionIndex = random.nextInt(action.size());
                        getswarm1=learn(actionIndex, R, Q, sub1.get(i), next,group);
                ...
                max[k]=sub1.get(i).getObjective(0)-getswarm1.getObjective(0)+(sub1.get(i).getObjective(1)-getswarm1.getObjective(1));
            ...
            else{swarmtemp.add(temp.get(best));}
            //swarmtemp.add(getswarm1);
        }
        group=2;
        for (int i = 0; i < centralSize; i++) {
            ...
            for (int k = 0; k < 30; k++) {
                ...
                if (k == 0) {
                    actionIndex = random.nextInt(action.size());
                    getswarm1=learn(actionIndex, R, Q, sub2.get(i), next,group);
                    int Qiannext = next;
                    if ((getswarm1.getObjective(0) < sub2.get(i).getObjective(0) && getswarm1.getObjective(1) < sub2.get(i).getObjective(1)) ||getswarm1.getObjective(0) < sub2.get(i).getObjective(0) ||getswarm1.getObjective(1) < sub2.get(i).getObjective(1)) next = 0;
                    else next = 1;

                    old0=sub2.get(i).getObjective(0);old1=sub2.get(i).getObjective(1);
                    new0=getswarm1.getObjective(0);new1=getswarm1.getObjective(1);
                    Q[Qiannext][actionIndex] = calculateNewQ1(R, Q,actionIndex,next,Qiannext,old0,old1,new0,new1);
                ...
                max[k]=sub2.get(i).getObjective(0)-getswarm1.getObjective(0)+(sub2.get(i).getObjective(1)-getswarm1.getObjective(1));
                temp.add(k,getswarm1);
            }
            int best=0;
            for(int y=1;y<30;y++){
        ...
        for (int i = 0; i < downSize; i++) {
            ...
            for (int k = 0; k < 30; k++) {
                ...
                if (k == 0) {
                    actionIndex = random.nextInt(action.size());
                    getswarm1=learn(actionIndex, R, Q, sub3.get(i), next,group);
                    int Qiannext = next;
                    if ((getswarm1.getObjective(0) < sub3.get(i).getObjective(0) && getswarm1.getObjective(1) < sub3.get(i).getObjective(1)) || getswarm1.getObjective(1) < sub3.get(i).getObjective(1) ||getswarm1.getObjective(0) < sub3.get(i).getObjective(0) ) next = 0;
                    else next = 1;

                    old0=sub3.get(i).getObjective(0);old1=sub3.get(i).getObjective(1);
                    new0=getswarm1.getObjective(0);new1=getswarm1.getObjective(1);
                    Q[Qiannext][actionIndex] = calculateNewQ1(R, Q,actionIndex,next,Qiannext,old0,old1,new0,new1);
                    //Q[Qiannext][actionIndex] = calculateNewQ(R, Q,actionIndex,next,Qiannext);
                } else {
                ...
                max[k]=sub3.get(i).getObjective(0)-getswarm1.getObjective(0)+(sub3.get(i).getObjective(1)-getswarm1.getObjective(1));
                temp.add(k,getswarm1);
            }
            int best=0;
            for(int y=1;y<30;y++){
...
// 几对交换序  就交换几次
    ...
    protected List<PermutationSolution<Integer>> PDDRFFselect(List<PermutationSolution<Integer>> swarm,
        ...
        for (int i = 0; i < swarm.size(); i++) {
            ...
            ArrayList<PermutationSolution<Integer>> A = new ArrayList<>(Pd.get(i).size());
            ArrayList<int[]> B = new ArrayList<int[]>();

            for (int j = 0; j < Pd.get(i).size(); j++) {

                A.add((PermutationSolution<Integer>) Pd.get(i).get(j).copy());
        ...
        for (int i = 0; i < swarm2.size(); i++) {
            ...
            ArrayList<PermutationSolution<Integer>> A = new ArrayList<>(Pd.get(DEswarmtempPdflag[i]).size());
            ArrayList<int[]> B = new ArrayList<int[]>();

            for (int j = 0; j < Pd.get(DEswarmtempPdflag[i]).size(); j++) {

                A.add((PermutationSolution<Integer>) Pd.get(DEswarmtempPdflag[i]).get(j).copy());
    ...
    protected List<PermutationSolution<Integer>> mager(List<PermutationSolution<Integer>> swarm1, List<PermutationSolution<Integer>> swarm2) {
     ...
     * @return
     */
    private ArrayList<SO> getDifferenceOfJobSequenceVectorByExchangeSequence(PermutationSolution<Integer> a, PermutationSolution<Integer> b) {     //  做差 得到交换序
    ...
    private void exchangeIndex4JobSequenceVectorByExchangeSequence(PermutationSolution<Integer> a, int index1, int index2) {     //交换值
    ...
    private void job(PermutationSolution<Integer> bb1, PermutationSolution<Integer> particle, double r, ArrayList<SO> list) {    //只动工件向量
    ...
    private void fac(PermutationSolution<Integer> bb1, PermutationSolution<Integer> particle, double r, ArrayList<SO> list) {     //只动工厂向量   交叉
    ...
    private PermutationSolution<Integer> DEJobFactory(List<PermutationSolution<Integer>> population, double DErate, int[] select) {    //工件工厂向量都操作
        PermutationSolution<Integer> childtemp = (PermutationSolution<Integer>) population.get(select[1]).copy();   //较差的粒子
        ArrayList<SO> listtemp = getDifferenceOfJobSequenceVectorByExchangeSequence(population.get(select[0]), population.get(select[1]));   // 得到的交换序
        Random A = new Random();
        double random1 = A.nextDouble();
    ...
    private PermutationSolution<Integer> DE2(List<PermutationSolution<Integer>> population, double DErate, int[] select) {    //只动工件向量
        PermutationSolution<Integer> childtemp = (PermutationSolution<Integer>) population.get(select[1]).copy();   //较差的粒子
    ...
    private PermutationSolution<Integer> DE2t(List<PermutationSolution<Integer>> population, int[] select) {    //只动工件向量
        ArrayList<ST> list = new ArrayList<>();
        PermutationSolution<Integer> childtemp = (PermutationSolution<Integer>) population.get(select[1]).copy();   //较差的粒子
        ST q = new ST(randomGenerator.nextInt(0, childtemp.getNumberOfVariables() - 1));
        list.add(q);
