...
public abstract class AbstractMOEADS<S extends Solution<?>> implements Algorithm< List<S>> {
  ...
  public AbstractMOEADS(Problem<S> problem, int populationSize, int resultPopulationSize,
...
//      }
//    }
    if ((problem.getNumberOfObjectives() == 7) && (populationSize <= 300)) {
      for (int n = 0; n < populationSize; n++) {
        double a = 1.0 * n / (populationSize - 1);
        lambda[n][0] = a;
        lambda[n][1] = 1 - 0.5* a;
        lambda[n][6] = 1 - lambda[n][0]-lambda[n][1];
      }
...
//    sleep();
  ...
  double fitnessFunction(S individual, double[] lambda) throws JMetalException {
    ...
    if (MOEADS.FunctionType.TCHE.equals(functionType)) {
      ...
      fitness = maxFun;
    } else if (MOEADS.FunctionType.AGG.equals(functionType)) {
      double sum = 0.0;
      for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
    ...
    } else if (MOEADS.FunctionType.PBI.equals(functionType)) {
      double d1, d2, nl;
      double theta = 5.0;
