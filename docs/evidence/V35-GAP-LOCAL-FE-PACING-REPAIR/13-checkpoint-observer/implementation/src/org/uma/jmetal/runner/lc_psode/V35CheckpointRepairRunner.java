...
/**
 * V35-GAP-LOCAL-FE-PACING-CHECKPOINT-V2 snapshot-bound experimental runner
 * (250k package).  Derived from V35LocalFePacingRepairRunner (V1) with exactly
 * two extensions: an optional pure-observation checkpoint observer
 * ({@code --observer ON}) wired through V35CheckpointObserverHook, and the
 * corresponding artifact export.  OFF mode replays the V1 runner semantics.
 *
 * <p>CLI: {@code --instance <name> --seed <long> --profile REF_A4|C0|C1|C2|C3
 * --max-fes <int> --snapshot <path> --output <path>
 * [--observer OFF|ON] [--checkpoints <csv of longs>]}.  Scientific parameters
 ...
 * local-FE budget betaMax.  The observer copies objective values and candidate
 * fingerprints only; it cannot enter the search archive, change PDDR inputs or
 * teacher selection, consume randomness, add evaluations, or mutate
 * candidates.</p>
 */
public final class V35CheckpointRepairRunner {
  public static final String VERSION = "v35-checkpoint-repair-runner-v2";
  ...
  private V35CheckpointRepairRunner() { }
  ...
  static void execute(Arguments value) throws Exception {
    ...
    Path experimentalJar = requireRegular(Paths.get("jars",
        "jmetal-algorithm-5.8-V35-LOCAL-FE-PACING-CHECKPOINT-V2.jar"));
    ...
    String runId = "GAPLCK-" + value.maxFes + "-" + label.cliAlias() + "-" + value.instance
        + "-" + value.seed;
    ...
    boolean moved = false;
    boolean observerArmed = value.observerOn;
    ...
    try {
      ...
      writeGate(partial, record, wallNanos, failures, runId, label, observerArmed,
          value.checkpoints, checkpoints.size(), observerErrors);
      ...
      System.out.println("V35_CHECKPOINT_REPAIR_COMPLETED profile=" + label.cliAlias()
          + " observer=" + (observerArmed ? "ON" : "OFF")
  ...
  /** Observer acceptance gate (250k package §5, ON side only). */
  ...
  /** Parsed CLI; the six permitted flags plus the observer toggle and targets. */
  static final class Arguments {
    ...
    static Arguments parse(String[] args) {
      ...
      Path snapshot = null;
      Path output = null;
      Boolean observerOn = Boolean.FALSE;
      ...
      for (int index = 0; index < args.length; index += 2) {
        ...
        else if ("--observer".equals(flag)) {
          if ("ON".equalsIgnoreCase(next)) observerOn = Boolean.TRUE;
          else if ("OFF".equalsIgnoreCase(next)) observerOn = Boolean.FALSE;
          else throw usage();
    ...
    private static IllegalArgumentException usage() {
      return new IllegalArgumentException(
          "usage: --instance <name> --seed <long> --profile REF_A4|C0|C1|C2|C3 "
              + "--max-fes <int> --snapshot <path> --output <path> "
              + "[--observer OFF|ON] [--checkpoints <csv>]");
    }
  }
}
