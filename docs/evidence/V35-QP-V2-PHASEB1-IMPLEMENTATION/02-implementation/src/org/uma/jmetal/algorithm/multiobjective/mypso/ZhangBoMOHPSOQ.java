...
public class ZhangBoMOHPSOQ extends AbstractParticleSwarmOptimization<PermutationSolution<Integer>, List<PermutationSolution<Integer>>> {
    ...
    private final Map<Long, ZhangBoPersonalLeaderDecision> pendingPersonalLeaders =
            new LinkedHashMap<>();
    private ZhangBoQpController customQpController;

    public void setCustomQpController(ZhangBoQpController customQpController) {
        this.customQpController = customQpController;
        if (customQpController != null) {
            this.zhangBoQpController = customQpController;
        }
    }

    public ZhangBoQpController getZhangBoQpController() {
        return zhangBoQpController;
    }
...
//        System.out.println("mutationRate4machine: " + mutationRate4machine);
    ...
    private void configureGlobalSearch(ZhangBoGlobalSearchConfiguration configuration) {
        ...
        if (configuration.isQpEnabled()) {
            if (customQpController != null) {
                zhangBoQpController = customQpController;
            } else {
...
//                mutation4worker(particle);
    ...
    private void selectQpGroup(
        ...
        if (zhangBoQpController != null) {
            zhangBoQpController.setObservationContext(generationNumber(), 0, fullEvaluationCount);
        }
