#!/bin/bash
# V35-SOURCE-ATTRIBUTION-500K / 10-v5-sa-normal-500k — SA-NORMAL V5 500k
# campaign=V35-SOURCE-ATTRIBUTION-V5-SA-NORMAL-500K
# runKey=SA-NORMAL-V5 instance=100_2_3_1 seed=20260901 arm=A4(profile C0) MaxFEs=500000 observer=ON
# Single JVM, nice -n 10, -Xms1g -Xmx4g (heap must NOT be enlarged; OOM => stop and report).
# Classpath order: V5 observer jar FIRST, formal jar SECOND.
# 19 configured checkpoints (25k..475k); the runner saves the terminal 500000 snapshot and B0 itself.
# Atomic publish (.partial-<name> -> <name>) is performed inside V35ObserverGateRunner.
set -uo pipefail
cd /home/inspur/aicomp/zhangbo-v35-source-attribution-v5-sa-normal-500k-20260902 || exit 1
mkdir -p results logs

CP="jars/jmetal-algorithm-5.8-V35-SOURCE-ATTRIBUTION-OBSERVER-V5.jar:jars/formal-algorithm-8DAD8F40.jar"
SNAP="snapshots/100_2_3_1-seed-20260901.fourvec"
OUT="results/SA-NORMAL-V5-500k"
...
{
  echo "campaign=V35-SOURCE-ATTRIBUTION-V5-SA-NORMAL-500K"
  echo "host=$(hostname)"
  ...
  echo "instanceSha256=$(sha256sum inputs/java-jmetal58/EADHFSP/100_2_3_1.txt | cut -d' ' -f1)"
  echo "setupFileSha256=$(sha256sum inputs/java-jmetal58/instance-extensions/v1/100_2_3_1.setup.txt | cut -d' ' -f1)"
  echo "fatigueFileSha256=$(sha256sum inputs/java-jmetal58/fatigue-parameters/v1/100_2_3_1.fatigue.txt | cut -d' ' -f1)"
...
echo "START SA-NORMAL-V5 $(date -Is)" | tee -a logs/run.log
nice -n 10 java -Xms1g -Xmx4g -cp "$CP" \
  org.uma.jmetal.runner.lc_psode.V35ObserverGateRunner \
  --instance 100_2_3_1 --seed 20260901 --profile C0 --max-fes 500000 \
  --snapshot "$SNAP" --output "$OUT" --telemetry ON --checkpoints "$CKPT" \
  > "logs/SA-NORMAL-V5.log" 2>&1
RC=$?
echo "END SA-NORMAL-V5 exit=$RC $(date -Is)" | tee -a logs/run.log
echo "$RC" > logs/exitcode.txt
{
  ...
  echo "logSizeBytes=$(stat -c %s logs/SA-NORMAL-V5.log)"
