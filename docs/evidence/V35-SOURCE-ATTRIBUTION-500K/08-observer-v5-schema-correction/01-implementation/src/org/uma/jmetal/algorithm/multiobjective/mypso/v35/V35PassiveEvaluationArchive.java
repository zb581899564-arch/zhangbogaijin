...
public final class V35PassiveEvaluationArchive implements Serializable {
  ...
  public void observe(PermutationSolution<Integer> evaluated) {
    observeWithSource(evaluated, null);
  }

  /**
   * V35-SOURCE-LEDGER-PATCH (2026-08-31): source-labelled observation. The
   * label is a pure tag produced by the caller at the evaluation site; no
   * behavior change, no RNG, no FE, no write-back.
   */
  @SuppressWarnings("unchecked")
  public void observeWithSource(PermutationSolution<Integer> evaluated,
      V35EvaluationSourceContext.Source source) {
    observeCore(evaluated);
    V35SourceAttributionObserver.onEvaluated(evaluated, source, observedCount);
    // V35-CHECKPOINT-OBSERVER-PATCH (2026-08-31): pure post-observation hook.
    // No-op unless armed; reads no RNG, consumes no FE, writes nothing back.
    V35CheckpointObserverHook.afterObserve(this);
  }

  private void observeCore(PermutationSolution<Integer> evaluated) {
