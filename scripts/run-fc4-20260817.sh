#!/usr/bin/env bash
# FC-3 pacing 基座 A/B + FC-4 rho 校准（训练机 2026-08-17）
set -euo pipefail
...
# FC-3 on the pacing base: A4+Pacing vs A4+Pacing+cheapTest, 20_2_3_1 50k x3.
for seed in 20260822 20260823 20260824; do
  echo "run_one 20_2_3_1 base  $seed 50000 \"--local-fe-budget 0.25:0.65\" fc3-pacing" >> "$LOG_ROOT/jobs.txt"
  echo "run_one 20_2_3_1 cheap $seed 50000 \"--local-fe-budget 0.25:0.65 --cheap-test true\" fc3-pacing" >> "$LOG_ROOT/jobs.txt"
done

# FC-4 rho calibration on the pacing base: {20,100} x {0.1,0.2,0.3} x 3 seeds, 500k.
for instance in 20_2_3_1 100_2_3_1; do
  for rho in 0.1 0.2 0.3; do
    for seed in 20260822 20260823 20260824; do
      echo "run_one $instance rho$rho $seed 500000 \"--local-fe-budget 0.25:0.65 --soft-freeze-rho $rho\" fc4-rho" >> "$LOG_ROOT/jobs.txt"
    done
  done
done
